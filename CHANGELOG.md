...
## Unreleased
- **Delete targets & assessments from the dashboard.** Red **Delete** buttons on target and assessment rows, protected by two-step confirmation (a per-row **Delete? Yes/No** safety switch plus a browser confirmation dialog) and a server-side `{"confirm": "YES"}` body requirement. Deleting a target removes its scope, assessments, findings, assets, services, evidence, checkpoints and job rows in one transaction; queued/running scans refuse deletion (409); every deletion is audit-logged (`TARGET_DELETE` / `ASSESSMENT_DELETE`). The dashboard page is now served with `Cache-Control: no-store` so UI updates always appear after a restart.
